...
module Api
  module V1
    class BookingsControllerTest < ActionDispatch::IntegrationTest
      ...
      test 'it should return error when api key is invalid' do
        post api_v1_bookings_url, params: @payload_one, as: :json

        assert_response :unauthorized
        assert_equal({ 'status' => 'error', 'message' => 'Request is unauthorized' }, response.parsed_body)
      end

      test 'it should successfully create booking' do
        post api_v1_bookings_url, headers: { 'Api-Key' => @api_key }, params: @payload_one, as: :json

        assert_response :created
      end

      test 'it should successfully create booking with different payload' do
        post api_v1_bookings_url, headers: { 'Api-Key' => @api_key }, params: @payload_two, as: :json

        assert_response :created
      end

      test 'it should return error when payload is empty' do
        post api_v1_bookings_url, headers: { 'Api-Key' => @api_key }, as: :json, params: nil
